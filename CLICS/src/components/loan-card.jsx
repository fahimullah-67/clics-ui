...
import { Card, CardContent, CardFooter } from "./custom-ui/Card";
...
export function LoanCard({ loan, onCompare, isComparing }) {
  ...
  return (
    <Card className="hover:shadow-lg transition-all h-full flex flex-col overflow-hidden border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
      ...
      {/* Gradient Header */}
      <div
        className={`bg-gradient-to-r ${loanTypeGradients[loan.type]} p-6 text-white relative`}
      >
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-2xl font-bold">
              {loan.interestRate.split("-")[0].trim()} p.a.
            </p>
          </div>
          ...
          {loan.id && (
            <Button
              ...
              onClick={() => onCompare && onCompare(loan.id)}
            >
              {isComparing ? (
      ...
      <CardContent className="flex-1 p-6 space-y-4 bg-white dark:bg-gray-900">
        ...
        <div className="flex items-center justify-between">
          <Badge
            className={`${loanTypeBadgeColors[loan.type]} border-0 px-3 py-1`}
            variant="secondary"
          >
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-500 dark:text-gray-400 w-28">
              Min. Deposit
            </span>
            <span className="font-medium text-gray-900 dark:text-white">
              {formatAmount(loan.minAmount)}
            </span>
          </div>
